...
平滑迁移参考链接：https://cloud.tencent.com/developer/article/1624911
...
对namesrv的配置，仅仅修改监听端口，默认为9876，建议修改至非默认端口

### 启动和关闭

> ```yaml
> 启动顺序，先启动nameserver，再启动broker，关闭时反之。
> ## 1、启动nameserver 
> nohup sh /data/soft/rocketmq-all-master/bin/mqnamesrv -c /data/soft/rocketmq-all-master/conf/nameserver.conf &
> ## 2、启动 broker
...
1，需要vim 编辑 rocketmq-console-ng-1.0.0.jar

2，查到到 application.properties，进行编辑

3，修改rocketmq.config.namesrvAddr为 namesrv的地址群、server.port为其他端口、rocketmq.config.dataPath为ROCKETMQ_HOME/data/ 目录
